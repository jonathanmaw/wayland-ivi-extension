...
destroy_ivicontroller_layer(struct wl_resource *resource)
{
    ...
    id_layer = ivi_controller_get_id_of_layer(shell, ivilayer->layout_layer);
    ...
    if ((ivilayer->layout_layer != NULL) &&
        ...
        ivi_controller_layer_remove(shell, ivilayer->layout_layer);
    }
}
...
get_surface(struct wl_list *list_surf, uint32_t id_surface)
{
    ...
    wl_list_for_each(ivisurf, list_surf, link) {
        ivisurf_id = ivi_controller_get_id_of_surface(ivisurf->shell, ivisurf->layout_surface);
...
get_layer(struct wl_list *list_layer, uint32_t id_layer)
{
    ...
    wl_list_for_each(ivilayer, list_layer, link) {
        ivilayer_id = ivi_controller_get_id_of_layer(ivilayer->shell, ivilayer->layout_layer);
                       ...
                       enum ivi_layout_notification_mask mask)
{
    ...
    ans = ivi_controller_get_layers_under_surface(shell, ivisurf->layout_surface,
                                                  &length, &pArray);
    if (0 != ans) {
    ...
    if (mask & IVI_NOTIFICATION_REMOVE) {
        ...
        for (i = 0; i < (int)length; i++) {
            ...
            id_layout_layer =
                ivi_controller_get_id_of_layer(shell, ivilayer->layout_layer);
                  ...
                  void *userdata)
{
    ...
    id_surface = ivi_controller_get_id_of_surface(shell, layout_surface);
                     ...
                     enum ivi_layout_notification_mask mask)
{
    ...
    ans = ivi_controller_get_screens_under_layer(shell, ivilayer->layout_layer,
                                                 &length, &pArray);
    if (0 != ans) {
                ...
                void *userdata)
{
    ...
    id_layout_layer = ivi_controller_get_id_of_layer(shell, layer);
    wl_list_for_each(ctrllayer, &shell->list_controller_layer, link) {
        if (id_layout_layer != ctrllayer->id_layer) {
            continue;
        }
                   ...
                   wl_fixed_t opacity)
{
    struct ivisurface *ivisurf = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_surface_set_opacity(ivisurf->shell, ivisurf->layout_surface, opacity);
}
                   ...
                   int32_t height)
{
    struct ivisurface *ivisurf = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_surface_set_source_rectangle(ivisurf->shell, ivisurf->layout_surface,
                     ...
                     int32_t height)
{
    ...
    ivi_controller_surface_set_transition(ivisurf->shell, ivisurf->layout_surface,
                                     IVI_LAYOUT_TRANSITION_NONE,
                                     300); // ms

    ivi_controller_surface_set_destination_rectangle(ivisurf->shell, ivisurf->layout_surface,
            (uint32_t)x, (uint32_t)y, (uint32_t)width, (uint32_t)height);
}
                      ...
                      uint32_t visibility)
{
    struct ivisurface *ivisurf = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_surface_set_visibility(ivisurf->shell, ivisurf->layout_surface, visibility);
}
                   ...
                   int32_t orientation)
{
    struct ivisurface *ivisurf = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_surface_set_orientation(ivisurf->shell, ivisurf->layout_surface, (uint32_t)orientation);
}
                      ...
                      const char *filename)
{
    ...
    prop = ivi_controller_get_properties_of_surface(shell, ivisurf->layout_surface);
    if (prop == NULL) {
                  ...
                  const char *filename)
{
    ...
    weston_surface = ivi_controller_surface_get_weston_surface(shell, ivisurf->layout_surface);
    if (weston_surface == NULL) {
        fprintf(stderr, "Failed to get weston surface.\n");
        return;
    }

    if (ivi_controller_surface_get_size(shell, ivisurf->layout_surface, &width, &height, &stride) != 0) {
                   ...
                   int32_t height)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_source_rectangle(ivilayer->shell, ivilayer->layout_layer,
           (uint32_t)x, (uint32_t)y, (uint32_t)width, (uint32_t)height);
}
                 ...
                 int32_t height)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_destination_rectangle(ivilayer->shell, ivilayer->layout_layer,
            (uint32_t)x, (uint32_t)y, (uint32_t)width, (uint32_t)height);
}
                    ...
                    uint32_t visibility)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_visibility(ivilayer->shell, ivilayer->layout_layer, visibility);
}
                 ...
                 wl_fixed_t opacity)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_opacity(ivilayer->shell, ivilayer->layout_layer, opacity);
}
                 ...
                 int32_t orientation)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_orientation(ivilayer->shell, ivilayer->layout_layer, (uint32_t)orientation);
}
                    ...
                    struct wl_resource *resource)
{
    struct ivilayer *ivilayer = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_layer_set_render_order(ivilayer->shell, ivilayer->layout_layer, NULL, 0);
}
                 ...
                 struct wl_resource *surface)
{
    ...
    (void)client;
    ivi_controller_layer_add_surface(ivilayer->shell, ivilayer->layout_layer, ivisurf->layout_surface);
}
                    ...
                    struct wl_resource *surface)
{
    ...
    struct ivisurface *ivisurf = wl_resource_get_user_data(surface);
    (void)client;
    ivi_controller_layer_remove_surface(ivilayer->shell, ivilayer->layout_layer, ivisurf->layout_surface);
}
                                  ...
                                  struct wl_array *id_surfaces)
{
    ...
    wl_array_for_each(id_surface, id_surfaces) {
        wl_list_for_each(ivisurf, &ivilayer->shell->list_surface, link) {
            id_layout_surface = ivi_controller_get_id_of_surface(shell, ivisurf->layout_surface);
    ...
    ivi_controller_layer_set_render_order(shell, ivilayer->layout_layer,
                                   layoutsurf_array, i);
              ...
              int32_t destroy_scene_object)
{
    ...
    struct ivicontroller_layer *ctrllayer = NULL;
    struct ivicontroller_layer *next = NULL;
    uint32_t id_layer = ivi_controller_get_id_of_layer(shell, ivilayer->layout_layer);
    (void)client;
    (void)destroy_scene_object;
                ...
                struct wl_resource *resource)
{
    struct iviscreen *iviscrn = wl_resource_get_user_data(resource);
    (void)client;
    ivi_controller_screen_set_render_order(iviscrn->shell, iviscrn->layout_screen, NULL, 0);
}
                ...
                struct wl_resource *layer)
{
    ...
    (void)client;
    ivi_controller_screen_add_layer(iviscrn->shell, iviscrn->layout_screen, ivilayer->layout_layer);
}
                ...
                const char *filename)
{
    ...
    output = ivi_controller_screen_get_output(iviscrn->shell, iviscrn->layout_screen);
                ...
                struct wl_array *id_layers)
{
    ...
    wl_array_for_each(id_layer, id_layers) {
        wl_list_for_each(ivilayer, &iviscrn->shell->list_layer, link) {
            id_layout_layer = ivi_controller_get_id_of_layer(shell, ivilayer->layout_layer);
    ...
    ivi_controller_screen_set_render_order(shell, iviscrn->layout_screen,
                                    layoutlayer_array, i);
                          ...
                          struct wl_resource *resource)
{
    ...
    ans = ivi_controller_commit_changes(controller->shell);
    if (ans < 0) {
                        ...
                        uint32_t id)
{
    ...
    if (ivilayer == NULL) {
        layout_layer = ivi_controller_layer_create_with_dimension(shell, id_layer,
    ...
    prop = ivi_controller_get_properties_of_layer(shell, ivilayer->layout_layer);
                          ...
                          uint32_t id)
{
    ...
    prop = ivi_controller_get_properties_of_surface(shell, ivisurf->layout_surface);
    ivi_controller_surface_set_content_observer(shell, ivisurf->layout_surface, surface_event_content, shell);
                        ...
                        struct ivicontroller *controller)
{
    ...
    wl_list_for_each_reverse(ivisurf, &shell->list_surface, link) {
        id_layout_surface =
            ivi_controller_get_id_of_surface(shell, ivisurf->layout_surface);
...
// TODO : Only Single display
    iviscrn->layout_screen = ivi_controller_get_screen_from_id(shell, 0);
             ...
             uint32_t id_layer)
{
    ...
    ivi_controller_layer_add_notification(shell, layout_layer, send_layer_prop, ivilayer);
               ...
               uint32_t id_surface)
{
    ...
    ivi_controller_surface_add_notification(shell, layout_surface,
                     ...
                     void *userdata)
{
    ...
    id_layer = ivi_controller_get_id_of_layer(shell, layout_layer);
                        ...
                        void *userdata)
{
    ...
    prop = ivi_controller_get_properties_of_surface(shell, layout_surface);
...
check_layout_layers(struct ivishell *shell)
{
    ...
    ret = ivi_controller_get_layers(shell, &length, &pArray);
    if(ret != 0) {
    ...
    for (i = 0; i < length; i++) {
        id_layer = ivi_controller_get_id_of_layer(shell, pArray[i]);
...
check_layout_surfaces(struct ivishell *shell)
{
    ...
    ret = ivi_controller_get_surfaces(shell, &length, &pArray);
    if(ret != 0) {
    ...
    for (i = 0; i < length; i++) {
        id_surface = ivi_controller_get_id_of_surface(shell, pArray[i]);
...
init_ivi_shell(struct weston_compositor *ec, struct ivishell *shell)
{
    ...
    ivi_controller_add_notification_create_layer(shell, layer_event_create, shell);
    ivi_controller_add_notification_remove_layer(shell, layer_event_remove, shell);

    ivi_controller_add_notification_create_surface(shell, surface_event_create, shell);
    ivi_controller_add_notification_remove_surface(shell, surface_event_remove, shell);

    ivi_controller_add_notification_configure_surface(shell, surface_event_configure, shell);
}
